...
- Desert temple/sand-buried ruin uses the `desertTemple` map style and is the final mock environment before shifting away from environment expansion. Its Level 2 renderer uses `fragmentedVertical` plus `linearRoute` metadata: exposed upper platforms, diagonal dune trails, sunken chambers, sand-choked thresholds, broken columns, carved slabs, blocked drifts, under-sand crawls, unstable lower stairs, and dust-worn masonry. It should not be treated as shrine or crypt with sand coloring; the structure should be shaped by burial depth, wind exposure, diagonal dune movement, and lower passages.
