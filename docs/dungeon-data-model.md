...
- `battleMapPrint`: optional future miniature-scale print calibration metadata.
...
Premium overlay anchors can use either absolute SVG coordinates (`x`, `y`) or percentage coordinates (`xPercent`, `yPercent`). Percentage coordinates are preferred for illustrated map tests because they align to `premiumMap.mapBounds` and are easier to retune as image size or print layout changes. Each label or marker anchor should provide either a complete `x`/`y` pair or a complete `xPercent`/`yPercent` pair. GM-only route overlay paths use the overlay SVG coordinate space, currently `0 0 720 480`.

The `720x480` overlay space is annotation/rendering space only. It is not physical print scale and cannot tell the app how many source pixels equal a map grid square. Accurate 1-inch miniature-scale battle-map printing requires `premiumMap.battleMapPrint` calibration. `battleMapPrint.status` can be `calibrated`, `uncalibrated`, or `unavailable`; calibrated maps must provide `grid.squareWidthPx` and `grid.squareHeightPx`, the source-image pixels per regular overlay grid column and row. Optional `grid.originXPercent`, `grid.originYPercent`, and `grid.rotationDeg` align the calibration grid visually. Optional `cropBoundsPercent` can define the playable grid area when it does not cover the full image, and optional `defaultOverlapInches` can suggest tiled-print overlap. Grid columns and rows should be derived from crop size and calibrated spacing, not manually required. The app overlay grid is the accurate future battle-print grid; baked-in illustrated grids may be slightly irregular, stretched, or obscured and should be treated as artwork. Current showcase maps are marked uncalibrated until they are visually calibrated in the annotator.
...
Surveyor/free schematic maps must still stay synchronized with dungeon content. For premium map-first dungeons, the simplified schematic view should derive keyed labels from the dungeon's premium label anchors, GM marker badges from `gmOverlay.markerAnchors` in GM mode, and route structure from `map.connections`, rather than falling back to a fixed environment template that may contain a different room count. Player mode must continue hiding GM markers and secret routes.

Premium map-first dungeons may also define `premiumMap.schematicFootprints` for Surveyor/free schematic maps. These footprints are simplified black-and-white room/area shapes that keep large illustrated areas, platforms, ledges, and clearings spatially representative without exposing premium art. Keep footprints clean and conservative: simple rectangles, capsules, or ovals with comfortable margins usually read better than highly rotated or oversized blobs. GM marker anchors should fall inside or clearly attach to the relevant footprint; player-safe schematic maps must still hide GM markers and secret routes.
