...
export function LevelTwoConnectionRoutes({
  ...
  isPlayer: boolean;
  theme: LevelTwoMapTheme;
  variant?: 'ruin' | 'crypt' | 'sewer' | 'laboratory';
}) {
...
export function LevelTwoRoomShell({
  ...
  const tileW = variant === 'crypt' || variant === 'sewer' || variant === 'laboratory' ? 30 : 28;
  const tileH = variant === 'crypt' ? 22 : 22;
  const cols = Math.ceil((w - 24) / tileW);
  const rows = Math.ceil((h - 24) / tileH);
  const horizontalBlocks = Math.max(2, Math.floor(w / (variant === 'crypt' || variant === 'sewer' || variant === 'laboratory' ? 30 : 28)));
  const verticalBlocks = Math.max(2, Math.floor(h / (variant === 'crypt' || variant === 'sewer' || variant === 'laboratory' ? 28 : 26)));
  ...
  return (
    <g>
      ...
      <rect x={x + 15} y={y + 15} width={w - 30} height={h - 30} rx="1" fill="none" stroke={variant === 'crypt' ? '#2d251f' : variant === 'sewer' ? '#1f302a' : variant === 'laboratory' ? '#3a302a' : '#4b3828'} strokeWidth="3" opacity={variant === 'crypt' ? '0.42' : '0.32'} />
