...
function ConnectionRoutes({
  ...
  return (
    <>
      <Corridors paths={normalPaths} kind={kind} />
...
function BlackfenLayout({ connections, palette, isPlayer }: { connections: MapConnection[]; palette: MapPalette; isPlayer: boolean }) {
  const roomNumbers = [
    { x: 130, y: 130, label: '1' },
    { x: 306, y: 122, label: '2' },
    { x: 149, y: 268, label: '3' },
  ...
  return (
    <>
      ...
      <ConnectionRoutes connections={connections} palette={palette} isPlayer={isPlayer} />
...
function ShrineLayout({ connections, palette, isPlayer }: { connections: MapConnection[]; palette: MapPalette; isPlayer: boolean }) {
  const roomNumbers = [
    { x: 154, y: 216, label: '1' },
    { x: 324, y: 204, label: '2' },
    { x: 480, y: 211, label: '3' },
...
function CavernLayout({ connections, palette, isPlayer }: { connections: MapConnection[]; palette: MapPalette; isPlayer: boolean }) {
  const roomNumbers = [
    { x: 96, y: 238, label: '1' },
    { x: 218, y: 174, label: '2' },
    { x: 222, y: 326, label: '3' },
  ...
  return (
    <>
      ...
      <ConnectionRoutes connections={connections} palette={palette} isPlayer={isPlayer} kind="natural" />
...
function CryptLayout({ connections, palette, isPlayer }: { connections: MapConnection[]; palette: MapPalette; isPlayer: boolean }) {
  const roomNumbers = [
    { x: 360, y: 89, label: '1' },
    { x: 360, y: 215, label: '2' },
    { x: 149, y: 217, label: '3' },
...
function SewerLayout({ connections, palette, isPlayer }: { connections: MapConnection[]; palette: MapPalette; isPlayer: boolean }) {
  const roomNumbers = [
    { x: 112, y: 238, label: '1' },
    { x: 280, y: 238, label: '2' },
    { x: 156, y: 112, label: '3' },
...
function LaboratoryLayout({ connections, palette, isPlayer }: { connections: MapConnection[]; palette: MapPalette; isPlayer: boolean }) {
  const roomNumbers = [
    { x: 145, y: 133, label: '1' },
    { x: 350, y: 109, label: '2' },
    { x: 569, y: 151, label: '3' },
...
function MapLayout({ mapData, style, palette, isPlayer }: { mapData?: DungeonMapData; style: MapStyle; palette: MapPalette; isPlayer: boolean }) {
  const connections = mapData?.connections ?? [];

  switch (style) {
    case 'shrine':
      return <ShrineLayout connections={connections} palette={palette} isPlayer={isPlayer} />;
    case 'cavern':
      return <CavernLayout connections={connections} palette={palette} isPlayer={isPlayer} />;
    case 'crypt':
      return <CryptLayout connections={connections} palette={palette} isPlayer={isPlayer} />;
    case 'sewer':
      return <SewerLayout connections={connections} palette={palette} isPlayer={isPlayer} />;
    case 'laboratory':
      return <LaboratoryLayout connections={connections} palette={palette} isPlayer={isPlayer} />;
    case 'blackfen':
    default:
      return <BlackfenLayout connections={connections} palette={palette} isPlayer={isPlayer} />;
  }
}
  ...
  return (
    <div className="overflow-hidden rounded-md border border-ink/10 bg-white shadow-tool">
      ...
      <svg viewBox="0 0 720 480" role="img" aria-label={`${isPlayer ? 'Player safe' : 'GM'} dungeon map`} className="h-auto w-full bg-[#efe7d6]">
        ...
        <MapLayout mapData={mapData} style={mapStyle} palette={palette} isPlayer={isPlayer} />
